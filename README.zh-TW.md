...
- macOS 15 或以上
- Xcode 26 或以上：在**執行時**需要，用於驅動 Simulator（SimPilot 會載入 Xcode 的 private Simulator frameworks）。安裝時不需要。
...
儲存的測試以確定性步驟驅動完整的互動面——點按、開關切換、滑桿、手勢、拖曳、長按、組合鍵與旋轉——而不只是點按與滑動。

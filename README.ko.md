...
`.simpilot/runs/`와 `.simpilot/verify/`를 프로젝트의 `.gitignore`에 추가하고 나머지는 추적하는 것을 권장합니다. `config.json`, `tests/`, `suites/`, 그리고 스킬이 앱 고유의 조작 특성을 기록하는 `notes.md`는 팀원이나 다른 에이전트에게 도움이 되는 부분입니다. `.simpilot/` 전체를 무시해도 개인 작업용으로는 동작하지만, 그 경우 아무것도 공유되지 않습니다.
...
- **오래 사용한 simulator는 키보드 HID를 받지 않게 될 수 있습니다**(Xcode 27.0 beta 6에서 측정). 붙여넣기, 키 단위 입력, 전체 선택+삭제가 모두 무시되지만 터치 입력은 동작합니다. 이는 iOS 버전이 아니라 기기의 사용 기간에 따르며 `simctl erase`나 재부팅으로도 복구되지 않습니다. `type`은 이 상태를 감지해 성공으로 보고하지 않고 실패합니다. `set-text`는 영향을 받지 않습니다. `type --xcode-mcp`은 대신 Xcode 27의 서비스로 입력하며, 측정한 기기에서는 그것으로 성공했지만 대가가 있습니다(아래 참고)
...
`sipi type --xcode-mcp`(테스트에서는 `"input-method": "xcode-mcp"`)로 이 경로를 선택합니다. SimPilot이 스스로 이 경로를 쓰는 일은 없습니다. iOS 27에서는 이 세션을 한 번 거치면 그 기기에서 이후 실행되는 모든 앱의 접근성 트리가 기기를 재시작(`simctl shutdown` + `boot`)할 때까지 비어 버리기 때문입니다. 마지막에 사용하거나, 다음 실행 전에 재시작하세요. SimPilot의 나머지 기능은 모두 이 설정 없이 동작하며, `set-text`는 애초에 키보드가 필요 없습니다.
...
프로젝트는 Xcode의 승인 대화상자를 띄우기 위해서만 열고 곧바로 닫습니다. 설정 후 서비스가 실행 중이 아니면 sipi가 시작합니다. Xcode는 승인을 정확한 바이너리에 묶으므로 `sipi update`나 재빌드 후에는 승인 절차를 다시 거쳐야 합니다. 현재 상태는 `sipi xcode-mcp`로, 이 경로의 사용 가능 여부는 `sipi doctor`로 확인할 수 있습니다.

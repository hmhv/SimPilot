...
- **長く使った simulator はキーボード HID を受け付けなくなることがあります**（Xcode 27.0 beta 6 で計測）。paste、キー単位の入力、select-all+delete はいずれも無視され、タッチ入力は動作します。これは iOS のバージョンではなくデバイスの使用期間に依存し、`simctl erase` でも再起動でも回復しません。`type` はこの状態を検出し、成功と報告せずに失敗します。`set-text` は影響を受けません。`type --xcode-mcp` は代わりに Xcode 27 のサービス経由で入力し、計測したデバイスではそれで成功しました。ただし代償があります（後述）
...
`sipi type --xcode-mcp`（テストでは `"input-method": "xcode-mcp"`）でこの経路を選びます。SimPilot が勝手にこの経路を使うことはありません。iOS 27 では、このセッションを1回行うと、そのデバイスで以後起動した App のアクセシビリティツリーがデバイス再起動（`simctl shutdown` + `boot`）まで空になるためです。最後に使うか、次の起動の前に再起動してください。これ以外の SimPilot の機能はすべてこの設定なしで動作し、`set-text` はそもそもキーボードを必要としません。
...
プロジェクトは Xcode の承認ダイアログを出すためだけに開き、すぐ閉じます。設定後、サービスが停止していれば sipi が起動します。Xcode は承認をバイナリ単位で紐付けるため、`sipi update` や再ビルドのたびに承認手順が必要です。現在の状態は `sipi xcode-mcp` で、この経路が使えるかどうかは `sipi doctor` で確認できます。

...
| Screenshot is black | Verify Booted state. Locked → `native_button home` if available, otherwise `axe button home`. Still launching → add sleep |
| App crashed | Check home screen with `ui_describe` → restart with `launch` → mark the step FAIL |
...
| Scroll position off | Use `axe swipe` to control amount. Verify with `ui_describe` after scrolling |
...
| System UI won't close | Try `ui_tap_label "<visible label>"` first. PhotosPicker multiple: `native_key 40` to confirm, `native_key 41` to cancel. FileImporter: `native_key 41`. ShareSheet: downward `axe swipe`. ColorPicker: touch the close button |

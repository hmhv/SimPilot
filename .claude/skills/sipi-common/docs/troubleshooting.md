...
| Typed text lands appended to what was already there | Both input methods insert at the caret. Use `sipi type --clear` (or `"clear": true` in a saved test) to select-all + delete first. `--clear` reaches committed text only — an IME's in-progress composition is not part of the value and survives it |
